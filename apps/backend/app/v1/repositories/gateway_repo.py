...
class GatewayRepository:
    ...
    async def get_by_uuid(self, gateway_uuid: str | uuid.UUID) -> Gateway | None:
        uuid_str = str(gateway_uuid)
        result = await self.session.execute(select(Gateway).where(Gateway.uuid == uuid_str))
    ...
    async def get_or_create(self, gateway_uuid: str, name: str = "edgegate") -> Gateway:
        ...
        if gw:
            return gw
        gw = Gateway(uuid=str(gateway_uuid), name=name, status="online")
        self.session.add(gw)
